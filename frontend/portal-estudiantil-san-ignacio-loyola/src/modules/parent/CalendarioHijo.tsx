...
const CalendarGrid = ({ currentDate, events, onViewEventDetails }: { currentDate: Date, events: ChildEvent[], onViewEventDetails: (event: ChildEvent) => void }) => {
...
const mockChildrenEvents: ChildEvents[] = [
  {
    childId: 'child1',
    ...
    events: [
      {
        ...
        color: 'red',
        description: 'Fecha límite para la entrega del ensayo sobre la Revolución Francesa. Asegúrate de subirlo a tiempo.',
      },
      {
        id: 'ce2',
        ...
        location: 'Aula 301',
        color: 'blue',
        description: 'Examen parcial de Matemáticas III. Repasa los temas de cálculo y álgebra lineal.',
      },
      {
        id: 'ce3',
        ...
        color: 'green',
        description: 'Clase de refuerzo para los temas más complejos de Química General. ¡No faltes!',
      },
    ],
  },
  {
    ...
    events: [
      ...
      },
      {
        ...
        displayDate: '05 Dic',
        time: '3:00 PM',
        location: 'Oficina del Tutor',
        color: 'blue',
        description: 'Reunión individual con el tutor para revisar el progreso académico y personal.',
      },
    ],
  },
];

export const CalendarioHijo = () => {
  const [selectedChildId, setSelectedChildId] = useState(mockChildrenEvents[0]?.childId || '');
  const [currentDate, setCurrentDate] = useState(new Date('2025-11-10')); // Start in November 2025 to show events
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedEventForDetails, setSelectedEventForDetails] = useState<ChildEvent | null>(null);
  ...
  const openEventDetailsModal = (event: ChildEvent) => {
  ...
  const currentChildEvents = mockChildrenEvents.find(
    (child) => child.childId === selectedChildId
  );

  // Filter events for the currently displayed month and selected child
  const eventsForMonth = currentChildEvents?.events.filter(event => {
  ...
  return (
    <Card className="p-6">
      <h1 className="text-3xl font-bold text-primary mb-8">Calendario de {currentChildEvents?.childName}</h1>

      {mockChildrenEvents.length > 1 && (
        <div className="mb-6">
